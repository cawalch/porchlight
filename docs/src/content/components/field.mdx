...
The `.pl-c-field` is a labeled form control. It pairs a visible label with a
...
Use `.pl-c-field--inline` when labels should scan in a vertical gutter, such as
...
Use `.pl-c-field__control` without a `.pl-c-field` wrapper when another component
owns the layout, as in command dialogs or toolbar searches. Keep an accessible
label. The control still receives theme tokens, density, validation, disabled
styling, and keyboard focus; use the wrapper for label/hint layout.
...
Use `.pl-c-field__required` for a visible marker when the product needs one. The
...
Use `.pl-c-field__hint` for one message, or `.pl-c-field__messages` when helper,
...
- **Required marker:** `.pl-c-field__required` is visual only and should be
...
- **Density:** sizing from `--pl-control-block-size`; `[data-pl-density]` on an
  ancestor.
...
- **Reduced motion:** the border/box-shadow transitions are zeroed by the
  themes layer.
- **Forced colors:** the border uses `ButtonBorder`; keyboard focus retains a
  two-pixel `Highlight` outline when box shadows are suppressed.
