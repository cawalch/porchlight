...
top-level `porchlight` wrap that's redundant: overrides live in your own
layers outside `porchlight` (or unlayered). One wrap, one mental model.

## Current status

This page documents the target architecture. The layers above are declared as
of this release; only `reset` is populated so far. Tokens, themes, and base
land next (Phase 1).
