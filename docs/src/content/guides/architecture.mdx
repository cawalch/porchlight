...
| `components`   | Component classes (`.pl-c-button`, `.pl-c-card`, …). |
...
`@scope (.pl-c-card) { … }`). These scoped rules look higher-specificity than a
flat `.pl-c-card { … }` rule in your app layer, but **layer order takes
