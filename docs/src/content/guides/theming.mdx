...
| **Component-local**             | `--pl-c-*`     | Per-component tokens defined inside `@scope` (e.g., `--pl-c-alert-tone`)                                       |
...
Omit the `data-pl-theme` attribute. The browser follows the OS setting:
...
Set `data-pl-theme` on the root element:
...
```html
<div data-pl-density="compact">Tighter controls</div>
<div data-pl-density="comfortable">Default</div>
<div data-pl-density="touch">Larger touch targets</div>
<div data-pl-density="dense">Maximum information density</div>
```
...
This is a token-level adjustment, not a separate component mode. Apps can
still override these tokens in their own layer when a brand needs different
contrast behavior.
...
Each component defines `--pl-c-*` tokens inside its `@scope`. Override these
...
@layer app {
  .pl-c-card {
    ...
    --pl-c-card-radius: var(--pl-radius-xl);
  }
}
```
